...
# 4. Auto-locate Paperklay install folder
# 5. Extract mod files in place
# 6. ViGEmBus install (REQUIRED for VR controllers)
#
# This is a motion-controls mod with VR gesture support:
# glider (both hands above head), punch attack, stomp
# attack (quick downward motion). ViGEm step is required
# for VR controller users.
#
# The config file ships in the ZIP, so no first-launch
# requirement like Silksong or Paranoia Place.
# -------------------------------------------------------
...
$DISCORD_DOWNLOAD_URL = "https://discord.com/channels/1001138422972432597/1380447945215836261/1388601628793508012"
$DISCORD_INFO_URL = "https://discord.com/channels/1001138422972432597/1380447945215836261/1380447945215836261"
...
function Write-Header {
 ...
 Write-Host " Paperklay VR Mod Installer" -ForegroundColor Cyan
...
function Find-PaperklayGamePath {
 $sp = Get-SteamPath
 if (-not $sp) { return $null }
 foreach ($lib in (Get-SteamLibraries -SteamPath $sp)) {
 # Unity data folder is "PaperKlay_Data" (camelCase). The
 # Steam installdir conventionally mirrors that. Note the
 # mod DLL itself is "Paperklay_VR.dll" with lowercase 'k'
...
Write-Host " Paperklay VR is distributed by Astienth via Discord." -ForegroundColor White
...
while ($alreadyMember -notin @("y","Y","n","N")) {
 ...
 Write-Host " 3. Download PaperKlay_VR_bHaptics.zip from the mod channel" -ForegroundColor Gray
...
Write-Host ""
Write-Host " This is a MOTION CONTROLS mod with VR gestures - both" -ForegroundColor White
Write-Host " hands above your head triggers the glider, a punch motion" -ForegroundColor White
Write-Host " triggers the attack, a quick downward motion in the air" -ForegroundColor White
Write-Host " triggers the stomp. To use VR controllers you MUST install" -ForegroundColor White
Write-Host " ViGEmBus (the installer can do it for you)." -ForegroundColor White
Write-Host ""
...
Write-Host " Once PaperKlay_VR_bHaptics.zip is on your disk, drop it here." -ForegroundColor Gray
Write-Host ""
...
# -------------------------------------------------------
# STEP 3: Locate the Paperklay install
# -------------------------------------------------------
Write-Step 2 4 "Locating PaperKlay"

$gamePath = Find-PaperklayGamePath
if (-not $gamePath) { $gamePath = Find-SteamGameFolder -AppId "1350720" -SteamFolderNames @("PaperKlay") }
if ($gamePath) {
 Write-OK "Found PaperKlay at: $gamePath"
} else {
 Write-Warn "Could not auto-locate PaperKlay in any Steam library."
 Write-Host " Please paste the path to your PaperKlay folder" -ForegroundColor White
 Write-Host " (the folder that contains 'PaperKlay_Data')." -ForegroundColor White
 Write-Host ""
 while (-not $gamePath) {
 $r = (Read-Host " PaperKlay folder").Trim().Trim('"')
 if (-not $r) { continue }
 if (Test-Path $r) {
 $gamePath = $r
...
$tempExtract = Join-Path $env:TEMP "PaperklayVR_$([System.IO.Path]::GetRandomFileName())"
...
$modDll = Join-Path $gamePath "BepInEx\plugins\Paperklay_VR.dll"
if (-not (Test-Path $bepinexDir)) {
 Write-Warn "BepInEx folder not found in $gamePath after copy."
} elseif (-not (Test-Path $modDll)) {
 Write-Warn "BepInEx is there but Paperklay_VR.dll is missing."
} else {
 Write-OK "BepInEx + Paperklay_VR.dll present."
}
...
Write-Host " Paperklay VR maps your VR controllers to a virtual" -ForegroundColor Gray
...
Write-Host "============================================================" -ForegroundColor Yellow
Write-Host ""
Write-Host " Launch PaperKlay normally via Steam, the desktop" -ForegroundColor White
...
Write-Host ""
Write-Host " VR MOTION GESTURES:" -ForegroundColor Cyan
Write-Host " Both hands above head (in air) -> Glider" -ForegroundColor Gray
Write-Host " Punch motion forward -> Attack" -ForegroundColor Gray
Write-Host " Quick downward motion (in air) -> Stomp attack" -ForegroundColor Gray
Write-Host ""
...
Write-Host " Edit BepInEx\config\Paperklay_VR.cfg" -ForegroundColor DarkGray
Write-Host " UseGestures = true (set false to disable motion" -ForegroundColor Gray
Write-Host " gestures and use only buttons)" -ForegroundColor Gray
Write-Host " LockUpDownCamera = true (set false to control camera" -ForegroundColor Gray
Write-Host " Y axis with right stick)" -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host " Folded worlds, soft heroes. Run wild." -ForegroundColor Magenta
Write-Host ""
