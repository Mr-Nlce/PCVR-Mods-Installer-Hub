...
VR port of Doom 2 via **GZDoomVR** - hh79's PC VR fork of GZDoom. Provides full 6DoF motion controls with laser-pointer aim, snap turn, and smooth locomotion through OpenVR / SteamVR.
...
That URL points at the **DOOM + DOOM II bundle**. DOOM2.WAD ships with this same combined package. (Older standalone Doom II app entries are no longer sold separately as of 2024.)

GOG owners are also supported - the installer will prompt for the WAD path if it can't find a Steam install.

## What this installer does

1. **Locates your WAD** - searches Steam for Doom 2 and copies `DOOM2.WAD` from there.
2. **Downloads the engine** - GZDoomVR v4.13.2.2 (~25 MB) from GitHub, on first install only.
3. **Creates a launcher** - `Start Doom 2 VR.bat` next to gzdoomvr.exe with VR mode pre-configured.
...
- Doom 2 owned on Steam (the WAD source) **or** a local `DOOM2.WAD` you can point the installer at.
- SteamVR running before launch.
- SteamVR Theatre Mode disabled: `Settings -> Dashboard -> Present Non-VR Applications` set to OFF.

## Performance notes

Vanilla Doom 2 runs comfortably on basic VR-capable hardware. Heavy mods (Brutal Doom and the like) can cause framedrops - the engine's README suggests dropping supersampling to 0.9 if you hit performance trouble.
...
>>> Hell came to Earth. Send it back.
