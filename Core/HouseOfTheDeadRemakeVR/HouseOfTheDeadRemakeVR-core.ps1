# -------------------------------------------------------
# The House of the Dead: Remake VR Mod Installer
...
# 4. Auto-locate House of the Dead Remake install folder
...
# virtual Xbox pad via ViGEmBus, with a 3D pistol attached
# to the right hand. ViGEm step is required for VR-controller
# users.
# -------------------------------------------------------
...
$MOD_NAME = "TheHouseOfTheDead_VR"
$MOD_VERSION = "v1.0.0"
$MOD_AUTHOR = "Astienth"

$GAME_APPID = "1694600"
$GAME_NAME = "The House of the Dead Remake"
...
function Write-Header {
 ...
 Write-Host " The House of the Dead Remake VR Mod Installer" -ForegroundColor Cyan
...
function Find-HOTDGamePath {
 $sp = Get-SteamPath
 if (-not $sp) { return $null }
 foreach ($lib in (Get-SteamLibraries -SteamPath $sp)) {
 # Unity data folder is "The House of the Dead Remake_Data"
 # (with spaces and capitalisation matching the title).
 # Steam installdir mirrors that. Try the standard form
 # first, then a few sensible variants.
 foreach ($folder in @(
 "The House of the Dead Remake",
 "THE HOUSE OF THE DEAD Remake",
 "TheHouseOfTheDeadRemake",
...
Write-Host " House of the Dead Remake VR is distributed by Astienth" -ForegroundColor White
...
while ($alreadyMember -notin @("y","Y","n","N")) {
 ...
 Write-Host " 3. Download TheHouseOfTheDeadRemake_VR.zip from the mod channel" -ForegroundColor Gray
...
Write-Host " This mod uses OpenXR by default. OpenVR is supported" -ForegroundColor White
Write-Host " via a config edit if you prefer." -ForegroundColor White
Write-Host ""
Write-Host " This is a MOTION CONTROLS mod with a 3D pistol attached" -ForegroundColor White
Write-Host " to your right hand. To use VR controllers you MUST" -ForegroundColor White
Write-Host " install ViGEmBus (the installer can do it for you)." -ForegroundColor White
Write-Host ""
Write-Host " Features included: bHaptics + ForceTube/Provolver support." -ForegroundColor White
Write-Host ""
...
Write-Host " Once TheHouseOfTheDeadRemake_VR.zip is on your disk, drop it here." -ForegroundColor Gray
Write-Host ""
...
# -------------------------------------------------------
# STEP 3: Locate the House of the Dead Remake install
# -------------------------------------------------------
Write-Step 2 4 "Locating House of the Dead Remake"

$gamePath = Find-HOTDGamePath
if (-not $gamePath) { $gamePath = Find-SteamGameFolder -AppId "1694600" -SteamFolderNames @("The House of the Dead Remake") -GogNames @("THE HOUSE OF THE DEAD Remake","House of the Dead Remake","The House of the Dead - Remake","The House of the Dead Remake") }
if ($gamePath) {
 Write-OK "Found House of the Dead Remake at: $gamePath"
} else {
 Write-Warn "Could not auto-locate House of the Dead Remake in any Steam library."
 Write-Host " Please paste the path to your House of the Dead Remake folder" -ForegroundColor White
 Write-Host " (the folder that contains 'The House of the Dead Remake_Data')." -ForegroundColor White
 Write-Host ""
 while (-not $gamePath) {
...
$tempExtract = Join-Path $env:TEMP "HOTDRemakeVR_$([System.IO.Path]::GetRandomFileName())"
...
$modDll = Join-Path $gamePath "BepInEx\plugins\TheHouseOfTheDead_VR.dll"
if (-not (Test-Path $bepinexDir)) {
 Write-Warn "BepInEx folder not found in $gamePath after copy."
} elseif (-not (Test-Path $modDll)) {
 Write-Warn "BepInEx is there but TheHouseOfTheDead_VR.dll is missing."
} else {
 Write-OK "BepInEx + TheHouseOfTheDead_VR.dll present."
}

# -------------------------------------------------------
# STEP 5: ViGEmBus driver (REQUIRED for VR controllers)
# -------------------------------------------------------
Write-Step 4 4 "ViGEmBus driver (REQUIRED for VR controllers)"

Write-Host " House of the Dead Remake VR maps your VR controllers to" -ForegroundColor Gray
Write-Host " a virtual Xbox gamepad through ViGEmBus. Without this" -ForegroundColor Gray
Write-Host " driver installed, the game won't accept your VR controller" -ForegroundColor Gray
...
Write-Host " - Aim with your right hand - 3D pistol model attached" -ForegroundColor Gray
Write-Host ""
Write-Host " IN-GAME:" -ForegroundColor Cyan
Write-Host " Right trigger -> Fire / shoot" -ForegroundColor Gray
Write-Host " Left trigger -> Reload" -ForegroundColor Gray
Write-Host " D-pad left/right -> Change weapon (D-pad via hotkey gesture)" -ForegroundColor Gray
Write-Host " Both stick clicks-> Recenter VR view" -ForegroundColor Gray
Write-Host " Right stick click + Y on left controller -> Force-recreate" -ForegroundColor Gray
Write-Host " pistol model" -ForegroundColor Gray
Write-Host ""
...
Write-Host " * Left stick = D-Pad (this is how you change weapons)" -ForegroundColor Gray
...
Write-Host " - ForceTube / Provolver support" -ForegroundColor Gray
Write-Host " - Left-handed mode (set in BepInEx\config\TheHouseOfTheDead_VR.cfg" -ForegroundColor Gray
Write-Host " AFTER the first launch with the mod):" -ForegroundColor Gray
Write-Host " leftHanded = true" -ForegroundColor Gray
Write-Host " Only the triggers swap; everything else is identical." -ForegroundColor Gray
Write-Host ""
Write-Host " Switch runtime to OpenVR (default is OpenXR):" -ForegroundColor Gray
Write-Host " Edit BepInEx\config\UnityVR_Bepinex.cfg" -ForegroundColor DarkGray
Write-Host " vrApi = OpenVR" -ForegroundColor Gray
Write-Host ""
Write-Host " Known issues:" -ForegroundColor Gray
Write-Host " - Pistol model sometimes doesn't appear: use right" -ForegroundColor Gray
Write-Host " stick click + Y on left controller to force recreate," -ForegroundColor Gray
Write-Host " or restart the game and it should appear on retry." -ForegroundColor Gray
Write-Host " - Level 1 monkey scene may have severely reduced visibility" -ForegroundColor Gray
Write-Host " (>2m). Just keep shooting - the monkey will go down." -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host " Reload. Aim. The dead are getting closer." -ForegroundColor Magenta
Write-Host ""
