# -------------------------------------------------------
# Mouse P.I. For Hire VR Mod Installer
...
# 4. Auto-locate Mouse install folder
...
function Write-Header {
 ...
 Write-Host " Mouse P.I. For Hire VR Mod Installer" -ForegroundColor Cyan
...
Write-Host " Mouse P.I. For Hire VR is distributed by Astienth via Discord." -ForegroundColor White
...
while ($alreadyMember -notin @("y","Y","n","N")) {
 ...
 Write-Host " 3. Download MousePI_VR.zip from the mod channel" -ForegroundColor Gray
...
Write-Host " This mod is OpenXR ONLY - make sure your headset's" -ForegroundColor White
Write-Host " OpenXR runtime is active before launching." -ForegroundColor White
Write-Host ""
...
Write-Host " Once MousePI_VR.zip is on your disk, drop it here." -ForegroundColor Gray
Write-Host ""
...
# -------------------------------------------------------
# STEP 3: Locate the Mouse install
# -------------------------------------------------------
Write-Step 2 4 "Locating Mouse"

$gamePath = Find-MouseGamePath
if (-not $gamePath) { $gamePath = Find-SteamGameFolder -AppId "2416450" -SteamFolderNames @("Mouse","MOUSE P.I. For Hire","MousePIForHire") }
if ($gamePath) {
 Write-OK "Found Mouse at: $gamePath"
} else {
 Write-Warn "Could not auto-locate Mouse in any Steam library."
 Write-Host " Please paste the path to your Mouse folder" -ForegroundColor White
 Write-Host " (the folder that contains 'MOUSE_Data')." -ForegroundColor White
 Write-Host ""
 while (-not $gamePath) {
 $r = (Read-Host " Mouse folder").Trim().Trim('"')
 if (-not $r) { continue }
 if (Test-Path $r) {
 $gamePath = $r
...
$tempExtract = Join-Path $env:TEMP "MousePIVR_$([System.IO.Path]::GetRandomFileName())"
...
$modDll = Join-Path $gamePath "BepInEx\plugins\MousePI_VR.dll"
if (-not (Test-Path $bepinexDir)) {
 Write-Warn "BepInEx folder not found in $gamePath after copy."
} elseif (-not (Test-Path $modDll)) {
 Write-Warn "BepInEx is there but MousePI_VR.dll is missing."
} else {
 Write-OK "BepInEx + MousePI_VR.dll present."
}
...
Write-Host " Mouse maps your VR controllers to a virtual" -ForegroundColor Gray
...
Write-Host "============================================================" -ForegroundColor Magenta
Write-Host ""
Write-Host " Launch MOUSE normally via Steam - it starts in VR (OpenXR only)." -ForegroundColor White
Write-Host ""
Write-Host " IMPORTANT - on first run, open the in-game settings and turn" -ForegroundColor Yellow
Write-Host " OFF aim assist, head bobbing and vsync for the best feel." -ForegroundColor Yellow
Write-Host ""
Write-Host " Quick controls:" -ForegroundColor White
Write-Host " - Recenter: click both sticks together, or press F1." -ForegroundColor Gray
Write-Host " - Weapon + aiming is on your DOMINANT hand; health on the other." -ForegroundColor Gray
Write-Host " - Turn with the right stick; weapon wheel = hold Y + right stick." -ForegroundColor Gray
Write-Host ""
Write-Host " Full controls, swimming and the config options are on this" -ForegroundColor Gray
Write-Host " game's page in the Hub." -ForegroundColor Gray
Write-Host ""
