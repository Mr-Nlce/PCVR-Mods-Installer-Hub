...
# 4. Auto-locate Astrodogs install folder
...
$DISCORD_DOWNLOAD_URL = "https://discord.com/channels/1001138422972432597/1424110040935043132/1425398903125315615"
$DISCORD_INFO_URL = "https://discord.com/channels/1001138422972432597/1424110040935043132/1424110255121367213"
...
function Write-Header {
 ...
 Write-Host " Astrodogs VR Mod Installer" -ForegroundColor Cyan
...
function Find-AstrodogsGamePath {
 $sp = Get-SteamPath
 if (-not $sp) { return $null }
 foreach ($lib in (Get-SteamLibraries -SteamPath $sp)) {
 # The Unity data folder is astrodogs_Data (lowercase) but
 # Steam installdir conventions usually mirror the store
 # title casing. We try the most likely variants in order.
 # Presence of astrodogs_Data is the strongest signal we
...
Write-Host " Astrodogs VR is distributed by Astienth via Discord." -ForegroundColor White
...
while ($alreadyMember -notin @("y","Y","n","N")) {
 ...
 Write-Host " 3. Download AstroDogs_VR.zip from the mod channel" -ForegroundColor Gray
...
Write-Host " This mod ships with OpenVR by default but supports" -ForegroundColor White
Write-Host " OpenXR via a config edit. Both runtimes work." -ForegroundColor White
Write-Host ""
Write-Host " This is a MOTION CONTROLS mod. To use your VR" -ForegroundColor White
Write-Host " controllers as input you MUST install ViGEmBus" -ForegroundColor White
Write-Host " (the installer can do it for you in step 6)." -ForegroundColor White
Write-Host " If you'd rather use a real gamepad or keyboard," -ForegroundColor White
Write-Host " ViGEmBus is not strictly required." -ForegroundColor White
Write-Host ""
...
Write-Host " Once AstroDogs_VR.zip is on your disk, drop it here." -ForegroundColor Gray
Write-Host ""
...
# -------------------------------------------------------
# STEP 3: Locate the Astrodogs install
# -------------------------------------------------------
Write-Step 2 4 "Locating Astrodogs"

$gamePath = Find-AstrodogsGamePath
if (-not $gamePath) { $gamePath = Find-SteamGameFolder -AppId "1301230" -SteamFolderNames @("Astrodogs") -GogNames @("Astrodogs") }
if ($gamePath) {
 Write-OK "Found Astrodogs at: $gamePath"
} else {
 Write-Warn "Could not auto-locate Astrodogs in any Steam library."
 Write-Host " Please paste the path to your Astrodogs folder" -ForegroundColor White
 Write-Host " (the folder that contains 'astrodogs_Data')." -ForegroundColor White
 Write-Host ""
 while (-not $gamePath) {
 $r = (Read-Host " Astrodogs folder").Trim().Trim('"')
 if (-not $r) { continue }
 if (Test-Path $r) {
 $gamePath = $r
...
$tempExtract = Join-Path $env:TEMP "AstrodogsVR_$([System.IO.Path]::GetRandomFileName())"
...
$modDll = Join-Path $gamePath "BepInEx\plugins\AstroDogs_VR.dll"
if (-not (Test-Path $bepinexDir)) {
 Write-Warn "BepInEx folder not found in $gamePath after copy."
} elseif (-not (Test-Path $modDll)) {
 Write-Warn "BepInEx is there but AstroDogs_VR.dll is missing."
} else {
 Write-OK "BepInEx + AstroDogs_VR.dll present."
}
...
Write-Host " Astrodogs maps your VR controllers to a virtual" -ForegroundColor Gray
...
Write-Host " input - you'd be stuck with a real gamepad or keyboard." -ForegroundColor Gray
Write-Host ""
...
Write-Host "============================================================" -ForegroundColor Magenta
Write-Host ""
Write-Host " Launch Astrodogs normally via Steam, the desktop" -ForegroundColor White
...
Write-Host ""
Write-Host " Real gamepad and keyboard also supported." -ForegroundColor Gray
Write-Host " OpenVR/SteamVR users can rebind any control via SteamVR" -ForegroundColor Gray
Write-Host " input bindings." -ForegroundColor Gray
Write-Host ""
Write-Host " Optional VR-hand ship control mode:" -ForegroundColor Gray
Write-Host " Edit BepInEx\config\AstroDogs_VR.cfg" -ForegroundColor DarkGray
Write-Host " VRShipControl = true (off by default)" -ForegroundColor Gray
Write-Host " LeftHand = true/false (which hand controls aiming)" -ForegroundColor Gray
Write-Host " HandMaxAngle = 0.4 (sensitivity, max 1.0)" -ForegroundColor Gray
Write-Host " offsetHand = 0,0,0 (rotation offset to recenter)" -ForegroundColor Gray
Write-Host " Live offset adjust in-game: hold left stick click + move" -ForegroundColor Gray
Write-Host " right stick. Left stick always overrides hand control." -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host " Bark at the stars. Bite the moons." -ForegroundColor Magenta
Write-Host ""
