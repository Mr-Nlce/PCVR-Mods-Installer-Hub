...
function global:New-GameCardClassic {
    ...
    elseif ($titleDisplay -eq "PowerSlave / Exhumed VR") {
        ...
        # Keep the generic suffix from becoming a line of its own.
    ...
    # subtree. Ordinary cards must not pay for hidden controls and handlers.
    $dualSplit = $null
    $dualCurrentBtn = $null
    $dualDepotBtn = $null
    ...
    if ($dualSplit) { $btnInner.Children.Add($dualSplit) | Out-Null }
