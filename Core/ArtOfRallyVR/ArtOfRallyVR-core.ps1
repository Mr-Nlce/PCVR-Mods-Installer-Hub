...
# 4. Auto-locate art of rally install folder
# 5. Extract mod files in place
#
# Art of Rally VR is a depth-only mod with an experimental
# first-person view toggle. The base game is gamepad or
# keyboard - no motion controls, no ViGEmBus needed.
#
# The mod author flagged this one as "quite barebone but
# playable" - it's a quick experimental build, not a polished
# full-featured mod. The heads-up below reflects that. There
# are also hidden hotkeys for tuning the first-person camera
# position that aren't in the main mod post but the author
# shared in follow-up comments.
#
# Naming pitfall worth noting:
# Game title: "art of rally" (all lowercase, three words)
# Steam folder: "artofrally" (lowercase, no spaces)
# Unity data: "artofrally_Data"
# Mod DLL: "ArtOfRally_VR.dll" (CamelCase + underscore)
# -------------------------------------------------------
...
$MOD_NAME = "ArtOfRally_VR"
$MOD_VERSION = "v1.0.0"
$MOD_AUTHOR = "Astienth"

$GAME_APPID = "550320"
$GAME_NAME = "art of rally"
...
$DISCORD_INFO_URL = "https://discord.com/channels/1001138422972432597/1306503565698662462/1306503565698662462"
$DISCORD_DOWNLOAD_URL = "https://discord.com/channels/1001138422972432597/1306503565698662462/1306504207012204585"
...
function Write-Header {
 ...
 Write-Host " Art of Rally VR Mod Installer" -ForegroundColor Cyan
...
function Find-ArtOfRallyGamePath {
 $sp = Get-SteamPath
 if (-not $sp) { return $null }
 foreach ($lib in (Get-SteamLibraries -SteamPath $sp)) {
 # Steam installdir is "artofrally" (lowercase, no spaces).
 # Game-developer convention from Funselektor Labs - matches
 # the EXE and Unity data folder names. We probe the title-
 # case + spaced form too as a courtesy in case some setup
 # ended up that way.
...
Write-Host " Art of Rally VR is distributed by Astienth via Discord." -ForegroundColor White
...
while ($alreadyMember -notin @("y","Y","n","N")) {
 ...
 Write-Host " 3. Download the Art of Rally VR ZIP from the mod post" -ForegroundColor Gray
...
# Heads-up about the depth-only / experimental nature.
...
Write-Host ""
Write-Host " Art of Rally VR is a DEPTH-ONLY mod with an experimental" -ForegroundColor White
Write-Host " first-person view toggle. The mod author labelled this" -ForegroundColor White
Write-Host " one as 'quite barebone but playable' - it's a quick" -ForegroundColor White
Write-Host " experimental build, not a polished feature-complete mod." -ForegroundColor White
Write-Host ""
Write-Host " Use a GAMEPAD or KEYBOARD - no VR controller support." -ForegroundColor White
...
Write-Host " Once the Art of Rally VR mod ZIP is on your disk, drop it here." -ForegroundColor Gray
Write-Host ""
...
# -------------------------------------------------------
# STEP 3: Locate the art of rally install
# -------------------------------------------------------
Write-Step 2 3 "Locating art of rally"

$gamePath = Find-ArtOfRallyGamePath
if (-not $gamePath) { $gamePath = Find-SteamGameFolder -AppId "550320" -SteamFolderNames @("artofrally") -GogNames @("art of rally") -EpicNames @("ArtOfRally") }
if ($gamePath) {
 Write-OK "Found art of rally at: $gamePath"
} else {
 Write-Warn "Could not auto-locate art of rally in any Steam library."
 Write-Host " Looking for a folder containing 'artofrally_Data'." -ForegroundColor Gray
 Write-Host " Please paste the path to your art of rally folder." -ForegroundColor White
 Write-Host ""
 while (-not $gamePath) {
 $r = (Read-Host " art of rally folder").Trim().Trim('"')
 if (-not $r) { continue }
 if (Test-Path $r) {
 $gamePath = $r
...
$tempExtract = Join-Path $env:TEMP "ArtOfRallyVR_$([System.IO.Path]::GetRandomFileName())"
...
$modDll = Join-Path $gamePath "BepInEx\plugins\ArtOfRally_VR.dll"
if (-not (Test-Path $bepinexDir)) {
 Write-Warn "BepInEx folder not found in $gamePath after copy."
} elseif (-not (Test-Path $modDll)) {
 Write-Warn "BepInEx is there but ArtOfRally_VR.dll is missing."
} else {
 Write-OK "BepInEx + ArtOfRally_VR.dll present."
}
...
Write-Host "============================================================" -ForegroundColor Magenta
Write-Host ""
Write-Host " Launch art of rally normally via Steam, the desktop" -ForegroundColor White
...
Write-Host " F1 -> Toggle first-person / third-person view" -ForegroundColor Gray
Write-Host " F2 -> Recenter VR view" -ForegroundColor Gray
Write-Host ""
Write-Host " ---- First-person camera tuning (hidden hotkeys) -------" -ForegroundColor Cyan
Write-Host ""
Write-Host " The mod author shared these in a follow-up comment - they" -ForegroundColor Gray
Write-Host " let you reposition the first-person camera while it's" -ForegroundColor Gray
Write-Host " active:" -ForegroundColor Gray
Write-Host ""
Write-Host " Hold LEFT CTRL + Arrow keys (up/down/left/right)" -ForegroundColor Gray
Write-Host " -> Move the camera on the horizontal plane" -ForegroundColor Gray
Write-Host ""
Write-Host " Hold LEFT CTRL + PageUp / PageDown" -ForegroundColor Gray
Write-Host " -> Move the camera on the vertical axis" -ForegroundColor Gray
Write-Host ""
Write-Host " Heads-up: this is a quick experimental mod. The author" -ForegroundColor Gray
Write-Host " describes it as 'barebone but playable'. F2 (recenter)" -ForegroundColor Gray
Write-Host " is documented as 'press once or twice, same thing' due" -ForegroundColor Gray
Write-Host " to a known quirk in the older universal-mod build it" -ForegroundColor Gray
Write-Host " was based on - not a problem, just a behaviour to know." -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host " Gravel, snow, sun. Trust the co-driver." -ForegroundColor Magenta
Write-Host ""
