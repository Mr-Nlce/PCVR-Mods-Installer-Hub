function global:Invoke-Tribes2VRElevatedOperation {
    [CmdletBinding()]
    param(
    ...
    $worker = Join-Path $PSScriptRoot 'Tribes2VRElevatedWorker.ps1'
    if (-not (Test-Path -LiteralPath $worker -PathType Leaf)) { throw 'The Tribes 2 VR elevation worker is missing.' }
    $requestRoot = Join-Path ([IO.Path]::GetTempPath()) ('pcvr_tribes2_admin_' + [Guid]::NewGuid().ToString('N'))
