# ============================================================
# Assassin's Creed Mirage VR - Installer (AnvilEngine2VR)
# ============================================================
...
$Host.UI.RawUI.WindowTitle = "Assassin's Creed Mirage VR Installer"
$ErrorActionPreference = "Stop"

$STEAM_APPID       = "3035570"
$GAME_STEAM_FOLDER = "Assassin's Creed Mirage"
$GAME_EXE          = "ACMirage.exe"
# Ubisoft+ / PC Game Pass installs ship a different exe name in the
# same Ubisoft folder - detect and launch whichever is present.
$GAME_EXE_PLUS     = "ACMirage_plus.exe"
...
$PINNED_URL_XR     = "https://github.com/mutars/anvilengine2vr/releases/download/v2.0.0.Public/Mirage-vr-openxr-v2.0.0.Public.zip"
$PINNED_URL_VR     = "https://github.com/mutars/anvilengine2vr/releases/download/v2.0.0.Public/Mirage-vr-openvr-v2.0.0.Public.zip"
...
function Write-Header {
 ...
 Write-Host " Assassin's Creed Mirage VR - Installer" -ForegroundColor Cyan
...
# -------------------------------------------------------
Write-Host " AnvilEngine2VR by mutars brings Assassin's Creed Mirage into 6DOF VR." -ForegroundColor White
Write-Host " Gamepad controls. Works with Steam, Ubisoft+ and Game Pass copies." -ForegroundColor White
Write-Host ""
Pause-User "Press Enter to start..."
Write-Step 1 5 "Locating Assassin's Creed Mirage"

$gamePath = $null

# Steam
if (Get-Command Try-FindSteamGame -ErrorAction SilentlyContinue) {
 $gamePath = Try-FindSteamGame -Folder $GAME_STEAM_FOLDER -Title "Assassin's Creed Mirage" -AppID $STEAM_APPID -ExeName $GAME_EXE
}
if (-not $gamePath -and (Get-Command Find-SteamGameFolder -ErrorAction SilentlyContinue)) {
 $gamePath = Find-SteamGameFolder -AppId $STEAM_APPID -SteamFolderNames @($GAME_STEAM_FOLDER) -ProbeExe $GAME_EXE `
   -EpicNames @("AssassinsCreedMirage", "Assassin's Creed Mirage")
}
...
if (-not $gamePath) {
 $candidates = @(
  "C:\Program Files (x86)\Ubisoft\Ubisoft Game Launcher\games\Assassin's Creed Mirage",
  "C:\Program Files\Epic Games\AssassinsCreedMirage",
  "D:\Program Files (x86)\Ubisoft\Ubisoft Game Launcher\games\Assassin's Creed Mirage",
  "E:\Program Files (x86)\Ubisoft\Ubisoft Game Launcher\games\Assassin's Creed Mirage"
 )
 foreach ($cand in $candidates) {
  if (Test-MirageRoot -Root $cand) { $gamePath = $cand; break }
 }
}
...
if (-not $gamePath) {
 ...
 if ($rec) { $rec = $rec.Trim() }
 if (Test-MirageRoot -Root $rec) { $gamePath = $rec }
}
# Manual fallback (typed or drag & dropped folder)
while (-not $gamePath) {
 Write-Warn "Could not find the game automatically."
 Write-Host " Drag & drop your Mirage GAME FOLDER onto this window (the one" -ForegroundColor White
 ...
 if (Test-MirageRoot -Root $raw) { $gamePath = $raw }
 else { Write-Fail "That folder does not contain $GAME_EXE / $GAME_EXE_PLUS." }
}

# Which exe is it? (Ubisoft+ / Game Pass ships ACMirage_plus.exe)
$exeName = $GAME_EXE
...
try {
 ...
 # Mirage asset for the chosen runtime, skipping the -symbols build
 # (debug symbols for crash reports - players do not need them).
 $asset = $rel.assets | Where-Object { $_.name -like "Mirage-vr-$variant*.zip" -and $_.name -notlike "*symbols*" } | Select-Object -First 1
 if ($asset) {
  $dlUrl = $asset.browser_download_url
  $relTag = [string]$rel.tag_name
  Write-Info "Latest release: $relTag"
 } else {
  Write-Warn "The latest release has no Mirage $variant build (it may be"
...
$zipPath = Join-Path $env:TEMP ("ACMVR_" + [System.IO.Path]::GetRandomFileName() + ".zip")
if (-not (Invoke-DownloadOrFallback -Url $dlUrl -Destination $zipPath -Label "AnvilEngine2VR ($variant)" `
   -ManualUrl $GITHUB_RELEASES `
   -Instructions "Download Mirage-vr-$variant-...zip (NOT the -symbols one) from the Releases page, drop it into your Downloads folder and retry.")) {
 $manualZip = Get-ChildItem -Path (Join-Path $env:USERPROFILE "Downloads") -Filter "Mirage-vr-$variant*.zip" -ErrorAction SilentlyContinue |
...
# Game Pass installs use ACMirage_plus.exe).
...
Write-Host ""
Write-Host " Nothing is true, everything is permitted. Welcome to Baghdad." -ForegroundColor Magenta
Write-Host ""
