...
# 4. Auto-locate Star Racer install folder
...
# is required for VR-controller users.
#
# Single-player only - split-screen is not supported by
# the VR mod.
# -------------------------------------------------------
...
$MOD_NAME = "StarRacer_VR"
$MOD_VERSION = "v1.0.0"
$MOD_AUTHOR = "Astienth"

$GAME_APPID = "2626120"
$GAME_NAME = "Star Racer"
...
$DISCORD_DOWNLOAD_URL = "https://discord.com/channels/1001138422972432597/1424109526621360240/1457285094082613461"
$DISCORD_INFO_URL = "https://discord.com/channels/1001138422972432597/1424109526621360240/1424109638558679142"
...
function Write-Header {
 ...
 Write-Host " Star Racer VR Mod Installer" -ForegroundColor Cyan
...
function Find-StarRacerGamePath {
 $sp = Get-SteamPath
 if (-not $sp) { return $null }
 foreach ($lib in (Get-SteamLibraries -SteamPath $sp)) {
 # Unity data folder is "Star Racer_Data" (with space).
 # Steam installdir mirrors that. Presence of the _Data
 # folder is the strongest signal we found the right
...
Write-Host " Star Racer VR is distributed by Astienth via Discord." -ForegroundColor White
...
while ($alreadyMember -notin @("y","Y","n","N")) {
 ...
 Write-Host " 3. Download StarRacer_VR.zip from the mod channel" -ForegroundColor Gray
...
Write-Host ""
Write-Host " Single-player ONLY. Split-screen is not supported by" -ForegroundColor White
Write-Host " the VR mod." -ForegroundColor White
Write-Host ""
...
Write-Host " Once StarRacer_VR.zip is on your disk, drop it here." -ForegroundColor Gray
Write-Host ""
...
# -------------------------------------------------------
# STEP 3: Locate the Star Racer install
# -------------------------------------------------------
Write-Step 2 4 "Locating Star Racer"

$gamePath = Find-StarRacerGamePath
if (-not $gamePath) { $gamePath = Find-SteamGameFolder -AppId "2626120" -SteamFolderNames @("Star Racer") }
if ($gamePath) {
 Write-OK "Found Star Racer at: $gamePath"
} else {
 Write-Warn "Could not auto-locate Star Racer in any Steam library."
 Write-Host " Please paste the path to your Star Racer folder" -ForegroundColor White
 Write-Host " (the folder that contains 'Star Racer_Data')." -ForegroundColor White
 Write-Host ""
 while (-not $gamePath) {
 $r = (Read-Host " Star Racer folder").Trim().Trim('"')
 if (-not $r) { continue }
 if (Test-Path $r) {
 $gamePath = $r
...
$tempExtract = Join-Path $env:TEMP "StarRacerVR_$([System.IO.Path]::GetRandomFileName())"
...
$modDll = Join-Path $gamePath "BepInEx\plugins\StarRacer_VR.dll"
if (-not (Test-Path $bepinexDir)) {
 Write-Warn "BepInEx folder not found in $gamePath after copy."
} elseif (-not (Test-Path $modDll)) {
 Write-Warn "BepInEx is there but StarRacer_VR.dll is missing."
} else {
 Write-OK "BepInEx + StarRacer_VR.dll present."
}
...
Write-Host " Star Racer maps your VR controllers to a virtual" -ForegroundColor Gray
...
Write-Host "============================================================" -ForegroundColor Yellow
Write-Host ""
Write-Host " Launch Star Racer normally via Steam, the desktop" -ForegroundColor White
Write-Host " shortcut, or the 'Start in VR' button in the Hub." -ForegroundColor White
Write-Host ""
Write-Host " Single-player only - split-screen is not supported." -ForegroundColor Yellow
Write-Host ""
...
Write-Host " LT + RT + X -> Recenter view" -ForegroundColor Gray
Write-Host " LB + RB + X -> Toggle 1st-person view (during" -ForegroundColor Gray
Write-Host " racing - pause first for comfort)" -ForegroundColor Gray
Write-Host " LB + RB + sticks -> Adjust camera offset:" -ForegroundColor Gray
Write-Host " left stick = move on horizontal plane" -ForegroundColor Gray
Write-Host " LT = down, RT = up" -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host " Hold the line. Pass on the inside. Win the galaxy." -ForegroundColor Magenta
Write-Host ""
