# -------------------------------------------------------
# Moros Protocol VR Mod Installer
...
# 4. Auto-locate Moros Protocol install folder
...
$MOD_NAME = "MorosProtocol_VR"
$MOD_VERSION = "v1.0.0"
$MOD_AUTHOR = "Astienth"

$GAME_APPID = "1605250"
$GAME_NAME = "Moros Protocol"
$GAME_EXE = "Moros Protocol.exe"
...
$DISCORD_DOWNLOAD_URL = "https://discord.com/channels/1001138422972432597/1456423903177211925/1456424479189373122"
$DISCORD_INFO_URL = "https://discord.com/channels/1001138422972432597/1456423903177211925/1456423903177211925"
...
function Write-Header {
 ...
 Write-Host " Moros Protocol VR Mod Installer" -ForegroundColor Cyan
...
function Find-MorosGamePath {
 $sp = Get-SteamPath
 if ($sp) {
 foreach ($lib in (Get-SteamLibraries -SteamPath $sp)) {
 # Game folder uses a literal space, and the _Data folder
 # is "Moros Protocol_Data" - presence of that subfolder
...
Write-Host " Moros Protocol VR is distributed by Astienth via Discord." -ForegroundColor White
...
while ($alreadyMember -notin @("y","Y","n","N")) {
 ...
 Write-Host " 3. Download MorosProtocol_VR.zip from the mod channel" -ForegroundColor Gray
...
Write-Host " Once MorosProtocol_VR.zip is on your disk, drop it here." -ForegroundColor Gray
Write-Host ""
...
# -------------------------------------------------------
# STEP 3: Locate the Moros Protocol install
# -------------------------------------------------------
Write-Step 2 4 "Locating Moros Protocol"

$gamePath = Find-MorosGamePath
if (-not $gamePath) { $gamePath = Find-SteamGameFolder -AppId "1605250" -SteamFolderNames @("Moros Protocol") -ProbeExe "Moros Protocol.exe" -GogNames @("Moros Protocol") }
if ($gamePath) {
 Write-OK "Found Moros Protocol at: $gamePath"
} else {
 Write-Warn "Could not auto-locate Moros Protocol in any Steam library."
 Write-Host " Please paste the path to your Moros Protocol folder" -ForegroundColor White
 Write-Host " (the folder that contains 'Moros Protocol_Data')." -ForegroundColor White
 Write-Host ""
 while (-not $gamePath) {
 $r = (Read-Host " Moros Protocol folder").Trim().Trim('"')
 if (-not $r) { continue }
 if (Test-Path $r) {
 $gamePath = $r
...
$tempExtract = Join-Path $env:TEMP "MorosProtocolVR_$([System.IO.Path]::GetRandomFileName())"
...
$modDll = Join-Path $gamePath "BepInEx\plugins\MorosProtocol_VR.dll"
if (-not (Test-Path $bepinexDir)) {
 Write-Warn "BepInEx folder not found in $gamePath after copy."
} elseif (-not (Test-Path $modDll)) {
 Write-Warn "BepInEx is there but MorosProtocol_VR.dll is missing."
} else {
 Write-OK "BepInEx + MorosProtocol_VR.dll present."
}
...
Write-Host " - All controls are rebindable in SteamVR Inputs" -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host " Config file: BepInEx\config\MorosProtocol_VR.cfg" -ForegroundColor Gray
Write-Host " isLeftHanded = false (true for left-hand mode)" -ForegroundColor Gray
Write-Host " flashlightsOnHead = true (false = on dominant hand)" -ForegroundColor Gray
Write-Host " handRotationOffsetX = 40 (aim angle; tune per controller)" -ForegroundColor Gray
Write-Host " UIPreset = curvedFollow" -ForegroundColor Gray
Write-Host " normal | curvedFixed | curvedFollow | noPreset" -ForegroundColor Gray
Write-Host ""
Write-Host " Known issues (per the modder):" -ForegroundColor Yellow
Write-Host " - Online co-op is NOT TESTED" -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host " Welcome to the Phobos. Try not to bleed on the consoles." -ForegroundColor Magenta
Write-Host ""
