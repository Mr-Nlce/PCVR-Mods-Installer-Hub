...
# 4. Auto-locate Sunrise GP install folder
# 5. Extract mod files in place
#
# This is a GAMEPAD/KEYBOARD mod - no VR controller support.
# The mod renders the cell-shaded racing game in VR but
# control input stays flat. No ViGEmBus needed because
# there's no VR-to-Xbox mapping.
# -------------------------------------------------------
...
$MOD_NAME = "SunriseGP_VR"
$MOD_VERSION = "v1.0.0"
$MOD_AUTHOR = "Astienth"

$GAME_APPID = "2670800"
$GAME_NAME = "Sunrise GP"
...
$DISCORD_DOWNLOAD_URL = "https://discord.com/channels/1001138422972432597/1362074365952528494/1374354399308283945"
$DISCORD_INFO_URL = "https://discord.com/channels/1001138422972432597/1362074365952528494/1362074365952528494"
...
function Write-Header {
 ...
 Write-Host " Sunrise GP VR Mod Installer" -ForegroundColor Cyan
...
function Find-SunriseGPGamePath {
 $sp = Get-SteamPath
 if (-not $sp) { return $null }
 foreach ($lib in (Get-SteamLibraries -SteamPath $sp)) {
 # Unity data folder is "Sunrise GP_Data" (with space).
...
Write-Host " Sunrise GP VR is distributed by Astienth via Discord." -ForegroundColor White
...
while ($alreadyMember -notin @("y","Y","n","N")) {
 ...
 Write-Host " 3. Download SunriseGP_VR.zip from the mod channel" -ForegroundColor Gray
...
Write-Host ""
Write-Host " Heads-up: there are some menu UI glitches (results screens" -ForegroundColor White
Write-Host " can overlap other menus) but the game stays playable." -ForegroundColor White
Write-Host ""
...
Write-Host " Once SunriseGP_VR.zip is on your disk, drop it here." -ForegroundColor Gray
Write-Host ""
...
# -------------------------------------------------------
# STEP 3: Locate the Sunrise GP install
# -------------------------------------------------------
Write-Step 2 3 "Locating Sunrise GP"

$gamePath = Find-SunriseGPGamePath
if (-not $gamePath) { $gamePath = Find-SteamGameFolder -AppId "2670800" -SteamFolderNames @("Sunrise GP") }
if ($gamePath) {
 Write-OK "Found Sunrise GP at: $gamePath"
} else {
 Write-Warn "Could not auto-locate Sunrise GP in any Steam library."
 Write-Host " Please paste the path to your Sunrise GP folder" -ForegroundColor White
 Write-Host " (the folder that contains 'Sunrise GP_Data')." -ForegroundColor White
 Write-Host ""
 while (-not $gamePath) {
 $r = (Read-Host " Sunrise GP folder").Trim().Trim('"')
 if (-not $r) { continue }
 if (Test-Path $r) {
 $gamePath = $r
...
$tempExtract = Join-Path $env:TEMP "SunriseGPVR_$([System.IO.Path]::GetRandomFileName())"
...
$modDll = Join-Path $gamePath "BepInEx\plugins\SunriseGP_VR.dll"
if (-not (Test-Path $bepinexDir)) {
 Write-Warn "BepInEx folder not found in $gamePath after copy."
} elseif (-not (Test-Path $modDll)) {
 Write-Warn "BepInEx is there but SunriseGP_VR.dll is missing."
} else {
 Write-OK "BepInEx + SunriseGP_VR.dll present."
}
...
Write-Host " - Some menu UI glitches (results screens overlapping" -ForegroundColor Gray
Write-Host " other menus). The game itself stays playable." -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host " Engines warm. Sun rising. Take pole position." -ForegroundColor Magenta
Write-Host ""
