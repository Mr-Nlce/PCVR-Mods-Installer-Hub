...
VR port of Strife via **GZDoomVR** - hh79's PC VR fork of GZDoom. Provides full 6DoF motion controls with laser-pointer aim, snap turn, and smooth locomotion through OpenVR / SteamVR.

## Where to get the game

The IWAD this installer copies comes from the official Steam release. Get it on Steam: https://store.steampowered.com/app/317040/

That URL points at the **Strife: Veteran Edition**. Still sold standalone on Steam. The Veteran Edition includes the original strife1.wad which is what this installer needs.

GOG owners are also supported - the installer will prompt for the WAD path if it can't find a Steam install.

## What this installer does

1. **Locates your WAD** - searches Steam for Strife and copies `strife1.wad` from there.
2. **Downloads the engine** - GZDoomVR v4.13.2.2 (~25 MB) from GitHub, on first install only.
3. **Creates a launcher** - `Start Strife VR.bat` next to gzdoomvr.exe with VR mode pre-configured.
...
- Strife owned on Steam (the WAD source) **or** a local `strife1.wad` you can point the installer at.
- SteamVR running before launch.
- SteamVR Theatre Mode disabled: `Settings -> Dashboard -> Present Non-VR Applications` set to OFF.

## Performance notes

Vanilla Strife runs comfortably on basic VR-capable hardware. Heavy mods (Brutal Doom and the like) can cause framedrops - the engine's README suggests dropping supersampling to 0.9 if you hit performance trouble.
...
>>> Join the Front. Topple the Order.
