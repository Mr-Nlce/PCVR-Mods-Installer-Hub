# -------------------------------------------------------
# The House of the Dead 2 Remake VR Mod Installer
...
# 4. Auto-locate House of the Dead 2 Remake install folder
...
# virtual Xbox pad via ViGEmBus. ViGEm step is required for
# VR-controller users.
#
# Laser pointer is built into this version (toggle in-game
# with right-stick click). No separate DLL variant.
# -------------------------------------------------------
...
$DISCORD_DOWNLOAD_URL = "https://discord.com/channels/1001138422972432597/1504155922572775554/1534909633393856512"
$DISCORD_INFO_URL = "https://discord.com/channels/1001138422972432597/1504155922572775554/1504155922572775554"
...
function Write-Header {
 ...
 Write-Host " The House of the Dead 2 Remake VR Mod Installer" -ForegroundColor Cyan
...
function Find-HOTD2GamePath {
 $sp = Get-SteamPath
 if (-not $sp) { return $null }
 foreach ($lib in (Get-SteamLibraries -SteamPath $sp)) {
 # Steam Community discussions on app/3376690 reference path
 # "Steamapps\common\THE HOUSE OF THE DEAD 2 Remake folder".
 # We try the verified casing first, then sensible variants.
 foreach ($folder in @(
 "THE HOUSE OF THE DEAD 2 Remake",
 "The House of the Dead 2 Remake",
 "TheHouseOfTheDead2Remake",
 "House of the Dead 2 Remake"
 )) {
 $candidate = Join-Path $lib "steamapps\common\$folder"
 if (Test-Path -LiteralPath "$candidate\THE HOUSE OF THE DEAD 2 Remake_Data") { return $candidate }
...
Write-Host " House of the Dead 2 Remake VR is distributed by Astienth" -ForegroundColor White
...
while ($alreadyMember -notin @("y","Y","n","N")) {
 ...
 Write-Host " 3. Download TheHouseOfTheDead2_VR.zip from the mod channel" -ForegroundColor Gray
...
Write-Host ""
Write-Host " If using SteamVR: launch SteamVR FIRST before launching the" -ForegroundColor White
Write-Host " game. The mod can have trouble starting otherwise." -ForegroundColor White
Write-Host ""
Write-Host " Features included: bHaptics (vest), in-game laser pointer." -ForegroundColor White
Write-Host ""
...
Write-Host " Once TheHouseOfTheDead2_VR.zip is on your disk, drop it here." -ForegroundColor Gray
Write-Host ""
...
# -------------------------------------------------------
# STEP 3: Locate the House of the Dead 2 Remake install
# -------------------------------------------------------
Write-Step 2 4 "Locating House of the Dead 2 Remake"

$gamePath = Find-HOTD2GamePath
if (-not $gamePath) { $gamePath = Find-SteamGameFolder -AppId "3376690" -SteamFolderNames @("THE HOUSE OF THE DEAD 2 Remake") -GogNames @("THE HOUSE OF THE DEAD 2 Remake","House of the Dead 2 Remake") }
if ($gamePath) {
 Write-OK "Found House of the Dead 2 Remake at: $gamePath"
} else {
 Write-Warn "Could not auto-locate House of the Dead 2 Remake in any Steam library."
 Write-Host " Please paste the path to your House of the Dead 2 Remake folder" -ForegroundColor White
 Write-Host " (the folder that contains 'THE HOUSE OF THE DEAD 2 Remake_Data')." -ForegroundColor White
 Write-Host ""
 while (-not $gamePath) {
...
$tempExtract = Join-Path $env:TEMP "HOTD2RemakeVR_$([System.IO.Path]::GetRandomFileName())"
...
$modDll = Join-Path $gamePath "BepInEx\plugins\TheHouseOfTheDead2_VR.dll"
if (-not (Test-Path $bepinexDir)) {
 Write-Warn "BepInEx folder not found in $gamePath after copy."
} elseif (-not (Test-Path $modDll)) {
 Write-Warn "BepInEx is there but TheHouseOfTheDead2_VR.dll is missing."
} else {
 Write-OK "BepInEx + TheHouseOfTheDead2_VR.dll present."
}
...
Write-Host " House of the Dead 2 Remake VR maps your VR controllers to" -ForegroundColor Gray
...
Write-Host " - Right trigger -> Fire (left trigger swaps in leftHanded mode)" -ForegroundColor Gray
Write-Host " - Right stick click -> Toggle laser pointer on/off" -ForegroundColor Gray
...
Write-Host " - bHaptics (vest) support" -ForegroundColor Gray
Write-Host " - In-game laser pointer (right-stick click toggle)" -ForegroundColor Gray
Write-Host " - Left-handed mode: edit BepInEx\config\TheHouseOfTheDead2_VR.cfg" -ForegroundColor Gray
Write-Host " after first launch -> leftHanded = true" -ForegroundColor Gray
Write-Host " (only triggers swap; everything else identical)" -ForegroundColor Gray
Write-Host ""
...
Write-Host " (OpenXR can interfere with gamepad controls - try OpenVR first)" -ForegroundColor DarkGray
Write-Host ""
...
Write-Host ""
Write-Host " Reload. Aim. The dead are back for round two." -ForegroundColor Magenta
Write-Host ""
