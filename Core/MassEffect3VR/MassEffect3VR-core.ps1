# ============================================================
# Mass Effect 3 VR - Installer (MELE3-VR by dhalcyon)
# ============================================================
# Mass Effect 3 Legendary Edition in full head-tracked PCVR:
# real stereo depth, 6DOF, three VR modes (Stereo / AER / DIBR,
# plus Mono) and an optional first-person camera. Gamepad only.
#
# The mod ships its OWN interactive setup batch (MELE3-VR.bat)
# that deliberately does NO game detection - it must sit in the
# game's ...\Game\ME3\Binaries\Win64 folder (next to
# MassEffect3.exe) and run FROM there; it self-elevates when the
...
$Host.UI.RawUI.WindowTitle = "Mass Effect 3 VR Installer"
...
$ME3_SUBPATH   = "Game\ME3\Binaries\Win64"
$ME3_EXE       = "MassEffect3.exe"
$LAUNCHER_SUB  = "Game\Launcher\MassEffectLauncher.exe"
$MOD_FILES     = @("MELE3-VR.bat", "dxgi.dll", "openxr_loader.dll", "MELE3VR.ini")
...
function Write-Header {
 ...
 Write-Host " Mass Effect 3 VR - Installer" -ForegroundColor Cyan
 Write-Host " MELE3-VR by dhalcyon | Legendary Edition (ME3) | gamepad" -ForegroundColor Gray
 Write-Host "============================================================" -ForegroundColor Magenta
 Write-Host ""
}
...
function Test-Mele2Root {
 ...
 $ok = $false
 try { $ok = Test-Path -LiteralPath ([System.IO.Path]::Combine($Root, $ME3_SUBPATH, $ME3_EXE)) } catch {}
 return $ok
}

Write-Header
Write-Host "  Mass Effect 3 Legendary Edition in full head-tracked VR with" -ForegroundColor Gray
...
while (-not $gamePath) {
 ...
 else { Write-Fail "That folder has no Game\ME3\Binaries\Win64\$ME3_EXE inside." }
}
if ($gamePath -match '(?i)\\steamapps\\common\\') { $isSteam = $true }
$win64 = [System.IO.Path]::Combine($gamePath, $ME3_SUBPATH)
...
# -------------------------------------------------------
Write-Step 2 5 "Getting MELE3-VR (Patreon)"
...
# really carries this game's payload: MELE3VR.zip inside the
# combined package, or MELE3-VR.bat in a single-game download.
$cands = @(Get-ChildItem -Path $dl -Filter "*.zip" -ErrorAction SilentlyContinue |
           Sort-Object LastWriteTime -Descending)
$found = $null
foreach ($c in $cands) {
    if ((Test-ArchiveContains -ArchivePath $c.FullName -Entry "MELE3VR.zip") -or
        (Test-ArchiveContains -ArchivePath $c.FullName -Entry "MELE3-VR.bat")) {
        $found = $c; break
    }
}
if ($found) {
...
if (-not $zip -and $DIRECT_URL) {
 $tmpZip = Join-Path $env:TEMP ("MELE3VR_" + [System.IO.Path]::GetRandomFileName() + ".zip")
 if (Invoke-DownloadOrFallback -Url $DIRECT_URL -Destination $tmpZip -Label "MELE3-VR" -ManualUrl $PATREON_URL `
    -Instructions "Download MELE3-VR.zip from the Patreon post, then retry.") { $zip = $tmpZip }
}
...
if (-not $zip) {
 ...
 if (-not $askedForPatreon) {
  ...
  Pause-User "Press Enter to open the Patreon post for MELE3-VR"
 }
 ...
 Write-Host " - Scroll to the post's attachment and download MELE3-VR.zip" -ForegroundColor Gray
...
# -------------------------------------------------------
Write-Step 3 5 "Placing the mod files next to $ME3_EXE"

$tmp = Join-Path $env:TEMP ("MELE3VR_x_" + [System.IO.Path]::GetRandomFileName())
New-Item -ItemType Directory -Path $tmp -Force | Out-Null
$exRes = Expand-ArchiveOrFallback -ArchivePath $zip -DestinationFolder $tmp -Label "MELE3-VR"
if (-not $exRes) {
...
$srcDir = Expand-NestedArchive -Root $tmp -Marker "MELE3-VR.bat" -Label "MELE3-VR"
if (-not $srcDir) {
 Write-Fail "MELE3-VR.bat is not in the download - neither loose nor in an inner archive."
 Pause-User "Press Enter to exit."; exit 1
}
$missing = @()
...
Write-Host " MELE3-VR brings its own interactive setup. It will ask:" -ForegroundColor White
...
Pause-User "Press Enter to start the MELE3-VR setup..."
try {
 Start-Process -FilePath (Join-Path $win64 "MELE3-VR.bat") -WorkingDirectory $win64 -Wait
 Write-OK "Mod setup finished."
} catch {
 Write-Warn "Could not run the setup automatically: $_"
 Write-Warn "Open $win64 and double-click MELE3-VR.bat yourself, then return here."
 Pause-User "Press Enter once the MELE3-VR setup is done..."
}
...
# Either way we do NOT point at MassEffect3.exe directly: that drags
...
Write-Host " The MELE3-VR setup has configured resolution and turned HDR" -ForegroundColor White
...
$others = @($ALL_GAMES | Where-Object { $_.Key -ne "ME3" -and (Test-Path -LiteralPath ([System.IO.Path]::Combine($gamePath, $_.Sub, $_.Exe))) })
...
Write-Host "     pick Mass Effect 3 in the launcher; the mod loads with it." -ForegroundColor White
Write-Host ""
