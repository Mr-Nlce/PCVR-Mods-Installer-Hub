# ============================================================
# Assassin's Creed Odyssey VR - Installer (AnvilEngine2VR)
# ============================================================
...
$Host.UI.RawUI.WindowTitle = "Assassin's Creed Odyssey VR Installer"
$ErrorActionPreference = "Stop"

$STEAM_APPID       = "812140"
# NOTE: the STEAM folder is "Assassins Creed Odyssey" - NO apostrophe
# (unlike the Ubisoft Connect folder, which has one). User-verified.
$GAME_STEAM_FOLDER = "Assassins Creed Odyssey"
$GAME_EXE          = "ACOdyssey.exe"
# Ubisoft+ / PC Game Pass installs ship a different exe name in the
# same Ubisoft folder - detect and launch whichever is present.
$GAME_EXE_PLUS     = "ACOdyssey_plus.exe"
...
$PINNED_URL_XR     = "https://github.com/mutars/anvilengine2vr/releases/download/v2.0.0.Public/Odyssey-vr-openxr-v2.0.0.Public.zip"
$PINNED_URL_VR     = "https://github.com/mutars/anvilengine2vr/releases/download/v2.0.0.Public/Odyssey-vr-openvr-v2.0.0.Public.zip"
...
function Write-Header {
 ...
 Write-Host " Assassin's Creed Odyssey VR - Installer" -ForegroundColor Cyan
...
if (Get-Command Try-FindSteamGame -ErrorAction SilentlyContinue) {
 $gamePath = Try-FindSteamGame -Folder $GAME_STEAM_FOLDER -Title "Assassin's Creed Odyssey" -AppID $STEAM_APPID -ExeName $GAME_EXE
}
if (-not $gamePath -and (Get-Command Find-SteamGameFolder -ErrorAction SilentlyContinue)) {
 $gamePath = Find-SteamGameFolder -AppId $STEAM_APPID -SteamFolderNames @($GAME_STEAM_FOLDER) -ProbeExe $GAME_EXE `
   -EpicNames @("AssassinsCreedOdyssey", "Assassin's Creed Odyssey")
}
...
if (-not $gamePath) {
 $candidates = @(
  "C:\Program Files (x86)\Ubisoft\Ubisoft Game Launcher\games\Assassin's Creed Odyssey",
  "C:\Program Files\Epic Games\AssassinsCreedOdyssey",
  "D:\Program Files (x86)\Ubisoft\Ubisoft Game Launcher\games\Assassin's Creed Odyssey",
  "E:\Program Files (x86)\Ubisoft\Ubisoft Game Launcher\games\Assassin's Creed Odyssey"
 )
 foreach ($cand in $candidates) {
  if (Test-OdysseyRoot -Root $cand) { $gamePath = $cand; break }
 }
}
...
if (-not $gamePath) {
 ...
 if ($rec) { $rec = $rec.Trim() }
 if (Test-OdysseyRoot -Root $rec) { $gamePath = $rec }
}
# Manual fallback (typed or drag & dropped folder)
while (-not $gamePath) {
 Write-Warn "Could not find the game automatically."
 Write-Host " Drag & drop your Odyssey GAME FOLDER onto this window (the one" -ForegroundColor White
 ...
 if (Test-OdysseyRoot -Root $raw) { $gamePath = $raw }
 else { Write-Fail "That folder does not contain $GAME_EXE / $GAME_EXE_PLUS." }
}

# Which exe is it? (Ubisoft+ / Game Pass ships ACOdyssey_plus.exe)
$exeName = $GAME_EXE
...
try {
 ...
 # Odyssey asset for the chosen runtime, skipping the -symbols build
 # (debug symbols for crash reports - players do not need them).
 $asset = $rel.assets | Where-Object { $_.name -like "Odyssey-vr-$variant*.zip" -and $_.name -notlike "*symbols*" } | Select-Object -First 1
 if ($asset) {
  $dlUrl = $asset.browser_download_url
  $relTag = [string]$rel.tag_name
  Write-Info "Latest release: $relTag"
 } else {
  Write-Warn "The latest release has no Odyssey $variant build (it may be"
...
$zipPath = Join-Path $env:TEMP ("ACOVR_" + [System.IO.Path]::GetRandomFileName() + ".zip")
if (-not (Invoke-DownloadOrFallback -Url $dlUrl -Destination $zipPath -Label "AnvilEngine2VR ($variant)" `
   -ManualUrl $GITHUB_RELEASES `
   -Instructions "Download Odyssey-vr-$variant-...zip (NOT the -symbols one) from the Releases page, drop it into your Downloads folder and retry.")) {
 $manualZip = Get-ChildItem -Path (Join-Path $env:USERPROFILE "Downloads") -Filter "Odyssey-vr-$variant*.zip" -ErrorAction SilentlyContinue |
...
# Game Pass installs use ACOdyssey_plus.exe).
...
Write-Host ""
Write-Host " Malaka! From Kephallonia to Olympus, the eagle soars." -ForegroundColor Magenta
Write-Host ""
