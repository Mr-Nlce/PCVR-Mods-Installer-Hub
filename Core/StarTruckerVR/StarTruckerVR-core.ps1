...
# Convenience: scan the Downloads folder for a likely Star Trucker VR
# zip (newest first) and offer it, so most users never touch drag-drop.
$dl = Join-Path $env:USERPROFILE "Downloads"
if (Test-Path $dl) {
 $cand = Get-ChildItem -Path $dl -Filter "*Star*Trucker*VR*.zip" -File -ErrorAction SilentlyContinue |
         Sort-Object LastWriteTime -Descending | Select-Object -First 1
 if (-not $cand) {
   $cand = Get-ChildItem -Path $dl -Filter "*Star*Trucker*.zip" -File -ErrorAction SilentlyContinue |
           Sort-Object LastWriteTime -Descending | Select-Object -First 1
 }
 if ($cand) {
