# -------------------------------------------------------
# Super Polygon Grand Prix VR Mod Installer
...
# 4. Auto-locate Super Polygon Grand Prix install folder
# 5. Extract mod files in place
#
# No VR controllers - gamepad/keyboard only - so no
# ViGEmBus setup step like Slyders/Moto Rush.
# -------------------------------------------------------
...
$MOD_NAME = "SuperPolygonGrandPrix_VR"
$MOD_VERSION = "v1.0.0"
$MOD_AUTHOR = "Astienth"

$GAME_APPID = "2459860"
$GAME_NAME = "Super Polygon Grand Prix"
$GAME_EXE = "Super Polygon Grand Prix.exe"
...
$DISCORD_DOWNLOAD_URL = "https://discord.com/channels/1001138422972432597/1492448070862901308/1492450946868056205"
$DISCORD_INFO_URL = "https://discord.com/channels/1001138422972432597/1492448070862901308/1492448247090778173"
...
function Write-Header {
 ...
 Write-Host " Super Polygon Grand Prix VR Mod Installer" -ForegroundColor Cyan
...
function Find-SPGPGamePath {
 $sp = Get-SteamPath
 if (-not $sp) { return $null }
 foreach ($lib in (Get-SteamLibraries -SteamPath $sp)) {
 # The Steam installdir is "SP-GP Super Polygon Grand Prix"
 # per SteamDB - the prefix "SP-GP " comes first. Try that
 # canonical name first, then sensible fallbacks.
 foreach ($folder in @(
 "SP-GP Super Polygon Grand Prix",
 ...
 # We can't be 100% sure of the exe name; just return
 # the folder if it exists - the user-confirm prompt
 # below will catch wrong picks.
 return $candidate
 }
 }
 }
...
Write-Host " Super Polygon Grand Prix VR is distributed by Astienth via Discord." -ForegroundColor White
...
while ($alreadyMember -notin @("y","Y","n","N")) {
 ...
 Write-Host " 3. Download SuperPolygonGrandPrix_VR.zip from the mod channel" -ForegroundColor Gray
...
Write-Host " Once SuperPolygonGrandPrix_VR.zip is on your disk, drop it here." -ForegroundColor Gray
Write-Host ""
...
# -------------------------------------------------------
# STEP 3: Locate the Super Polygon Grand Prix install
# -------------------------------------------------------
Write-Step 2 4 "Locating Super Polygon Grand Prix"

$gamePath = Find-SPGPGamePath
if (-not $gamePath) { $gamePath = Find-SteamGameFolder -AppId "2459860" -SteamFolderNames @("SP-GP Super Polygon Grand Prix") }
if ($gamePath) {
 Write-OK "Found Super Polygon Grand Prix at: $gamePath"
} else {
 Write-Warn "Could not auto-locate Super Polygon Grand Prix in any Steam library."
 Write-Host " Please paste the path to your Super Polygon Grand Prix install folder." -ForegroundColor White
 Write-Host " (The folder that contains the game .exe)" -ForegroundColor Gray
 Write-Host ""
 while (-not $gamePath) {
 $r = (Read-Host " Super Polygon Grand Prix folder").Trim().Trim('"')
 if (-not $r) { continue }
 if (Test-Path $r) {
 # Loose check - just confirm it's a folder. The mod
 # readme says it should have BepInEx at root after
 # extract; we trust the user's path here.
 $gamePath = $r
...
$tempExtract = Join-Path $env:TEMP "SuperPolygonGrandPrixVR_$([System.IO.Path]::GetRandomFileName())"
...
# Sanity check that BepInEx landed
...
if (-not (Test-Path $bepinexDir)) {
 ...
 Write-Host " Look inside the game folder to verify." -ForegroundColor Gray
} else {
 Write-OK "BepInEx folder present in game directory."
}
...
Write-Host " - HOLD the START button to recenter the view." -ForegroundColor Gray
Write-Host ""
Write-Host " Known limitation:" -ForegroundColor Yellow
Write-Host " - The pause menu is not visible in VR (modder note)." -ForegroundColor Gray
Write-Host ""
...
Write-Host ""
Write-Host " Sharp corners. Sharper edges. Floor it." -ForegroundColor Magenta
Write-Host ""
